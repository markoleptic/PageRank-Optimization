...
static void baseline_matrix_mul(int start, int end, multiformat_graph_t *graph, pagerank_data_t *pagerank, float *array)
{
	for (int cur_pos = start; cur_pos < end; ++cur_pos)
	{
		...
		array[row_idx] += val * pagerank->x[col_idx];
	}
}
...
#pragma omp parallel num_threads(num_threads) reduction(+:y_accumulated[:m])
	{
#pragma omp for
...
#pragma omp for
		...
		{
			baseline_matrix_mul(indices->reg_indices[i].start, indices->reg_indices[i].end, graph,
					    pagerank_data, y_accumulated);
		}
	}
    for (int i = 0; i < m; ++i)
    {
