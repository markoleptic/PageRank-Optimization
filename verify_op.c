...
extern void COMPUTE_NAME_REF( int num_vertices, int num_iterations,
			      multiformat_graph_t *multiformat_graph_distributed,
			      pagerank_data_t *pagerank_data_distributed );

extern void COMPUTE_NAME_TST( int num_vertices, int num_iterations,
			      multiformat_graph_t *multiformat_graph_distributed,
			      pagerank_data_t *pagerank_data_distributed );
...
extern void COLLECT_DATA_NAME_REF( int num_vertices, int num_iterations,
				   pagerank_data_t *pagerank_data_distributed,
				   pagerank_data_t *pagerank_data_sequential );

extern void COLLECT_DATA_NAME_TST( int num_vertices, int num_iterations,
				   pagerank_data_t *pagerank_data_distributed,
				   pagerank_data_t *pagerank_data_sequential );
...
int main( int argc, char *argv[] )
{
  ...
  for( int p = min_size;
	...
	{ /* root node */
	  ...
	  float kron_stochastic_buf[] = {0.91f,0.75f,
					 0.63f,0.52f};
	  ...
	  coo_matrix_t *coo_graph =
	    create_coo_matrix_with_kronecker_random_data(num_vertices,num_vertices,
	...
	Run the reference
      */
      ...
      DISTRIBUTED_ALLOCATE_NAME_REF( num_vertices, num_iterations,
				     &multiformat_graph_distributed_ref,
				     &pagerank_data_distributed_ref );

      // Distribute the sequential buffers 
      DISTRIBUTE_DATA_NAME_REF( num_vertices, num_iterations,
				multiformat_graph_sequential_ref,
				pagerank_data_sequential_ref,
				multiformat_graph_distributed_ref,
				pagerank_data_distributed_ref );
     
      // Perform the computation
      COMPUTE_NAME_REF( num_vertices, num_iterations,
			multiformat_graph_distributed_ref,
			pagerank_data_distributed_ref );


      // Collect the distributed data and write it to a sequential buffer
      COLLECT_DATA_NAME_REF( num_vertices, num_iterations,
			     pagerank_data_distributed_ref,
			     pagerank_data_sequential_ref );     
     
      // Finally free the buffers
      DISTRIBUTED_FREE_NAME_REF( num_vertices, num_iterations,
				 multiformat_graph_distributed_ref,
				 pagerank_data_distributed_ref );
      ...
      // Allocate distributed buffers for the reference
      DISTRIBUTED_ALLOCATE_NAME_TST( num_vertices, num_iterations,
				     &multiformat_graph_distributed_tst,
				     &pagerank_data_distributed_tst );

      // Distribute the sequential buffers 
      DISTRIBUTE_DATA_NAME_TST( num_vertices, num_iterations,
				multiformat_graph_sequential_tst,
				pagerank_data_sequential_tst,
				multiformat_graph_distributed_tst,
				pagerank_data_distributed_tst );
     
      // Perform the computation
      COMPUTE_NAME_TST( num_vertices, num_iterations,
      			multiformat_graph_distributed_tst,
      			pagerank_data_distributed_tst );


      // Collect the distributed data and write it to a sequential buffer
      COLLECT_DATA_NAME_TST( num_vertices, num_iterations,
			     pagerank_data_distributed_tst,
			     pagerank_data_sequential_tst);     
     
      // Finally free the buffers
      DISTRIBUTED_FREE_NAME_TST( num_vertices, num_iterations,
				 multiformat_graph_distributed_tst,
				 pagerank_data_distributed_tst );
	...
	{
	  /* root node */
	  float res = max_pair_wise_diff(num_vertices,1,1,1,
					 pagerank_data_sequential_ref->y,
					 pagerank_data_sequential_tst->y);
	  
	  fprintf(result_file, "%i,%i,%i,%f,",
		  num_ranks,
		  num_vertices,num_iterations,
		  res);
